...
from config import DEFAULT_TABLE_BUS, SAVE_DEFAULT_TABLE_TROLLEYBUSES, FONT_PNG, MESSAGE_KEYBOARD
...
class Scene(BaseScene):
    async def message_handler(self, message: Message):
        ...
        temporary_data = await self.getter.get_data_bus(city=_data.city, type_stop='dirty', type_transport='trolleybuses', transport_number=text, time=_data.time, sort=_data.sort)
        if text == MESSAGE_KEYBOARD['back_keyb']:
            ...
            await self.bot.send_message(message.from_user.id, self.lang['not_trolleibys'], reply_markup=self.keyboard.kontroler_bus())
        elif temporary_data:
            if _data.display == 'Фото':
                img_busstop(name_png=SAVE_DEFAULT_TABLE_TROLLEYBUSES, _dict=temporary_data, cordinates_x=(60, 650, 1250), cordinate_y=45, y_step=92,
