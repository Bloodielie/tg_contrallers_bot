...
class Scene(BaseScene):
    async def message_handler(self, message: Message):
        ...
        SAVE_DEFAULT_TABLE_BUS = config.get_data('pillow', 'SAVE_DEFAULT_TABLE_BUS')
        text = message.text
        ...
        if text == MESSAGE_KEYBOARD['back_keyb']:
            ...
            keyb = create_keyboard(kontroler_bus)
            await message.answer(MESSAGE['not_bus'], reply_markup=keyb)
        elif temporary_data:
            keyb = create_keyboard(kontroler_bus)
            if data.display.lower() == 'фото':
                img_busstop(name_png=SAVE_DEFAULT_TABLE_BUS, _data=temporary_data, name_png_first=DEFAULT_TABLE_BUS)
                await message.answer_photo(InputFile('png/tablebus.gif'), reply_markup=keyb)
            else:
