...
class Scene(BaseScene):
    async def message_handler(self, message: Message):
        ...
        if text == MESSAGE_KEYBOARD['settings_keyb_display_inside_1'].lower():
            self.user_data[0].display = text
            await self.manager.update(self.user_data[0])
            await message.answer(MESSAGE['settings_display_inside_msg'].format(display='картинками'), reply_markup=keyb)
        elif text == MESSAGE_KEYBOARD['settings_keyb_display_inside_2'].lower():
            self.user_data[0].display = 'Сообщения'
            await self.manager.update(self.user_data[0])
            await message.answer(MESSAGE['settings_display_inside_msg'].format(display='текстом'), reply_markup=keyb)
